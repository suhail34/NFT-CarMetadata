...
/// - short_description: [String] 
	pub fn process_mint(
		...
		let car_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (car_pubkey, car_bump) = Pubkey::find_program_address(
			&[b"car", mint_info.key.as_ref()],
			program_id,
		);
		...
		invoke_signed(
			...
			&[fee_payer_info.clone(), car_info.clone()],
			&[&[b"car", mint_info.key.as_ref(), &[car_bump]]],
		)?;
		...
		if car_info.data_len() != CarMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let car = &mut AccountPDA::new(
			&car_info,
			try_from_slice_unchecked::<CarMetadata>(&car_info.data.borrow()).unwrap(),
			car_bump,
		);
		...
		car.data.serialize(&mut &mut car_info.data.borrow_mut()[..])?;
